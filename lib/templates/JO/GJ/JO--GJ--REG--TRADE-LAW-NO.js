    ...
    try {
...

        {
            ...
            var tags = await page.evaluate(function process(url) {
                ...
                try { tags.push("<div class='level1' title='level1'>" + elements[0].outerHTML + "</div>"); elements.remove()} catch (e) { }
                                    ...
                                    if(articleChilds.className == "article-num"){
                                        ...
                                        for(const articleChild of articleChilds.childNodes){
                                            if(articleChild.nodeName == "P"){
                                                const plainText = articleChild.innerHTML.replace(/<br>/g, '<p>');
                                                try { tags.push("<p>" + plainText + "</p>"); } catch (e) { }
                                            }
                                        }
                                    }
                                }
