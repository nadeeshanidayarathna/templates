        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                for (const level2Element of level2Elements) {
                    if (level2Element.textContent.trim().indexOf('、') == 1 && !(level2Element.textContent.trim().startsWith('九') || level2Element.textContent.trim().startsWith('十'))) {
                        wrapElement(document, level2Element, "level2");
                    } else if (level2Element.textContent.trim().startsWith('九') || level2Element.textContent.trim().startsWith('十')) {
                        let level2Title = /\W、/.exec(level2Element.outerHTML);
                        level2Element.outerHTML = level2Element.outerHTML.replace(level2Title[0] , "<div class=\"level2\" title=\"level2\">" + level2Title[0] + "</div>")
                    }
                }
