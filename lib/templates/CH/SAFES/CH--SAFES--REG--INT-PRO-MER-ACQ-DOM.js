        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                for (const element of elements) {
                    if (element.textContent.trim().charAt(0) == "第" && element.textContent.trim().charAt(2) == "条") {
                        const level2Element = /.*第.*\d.*条/.exec(element.outerHTML);
                        element.outerHTML = element.outerHTML.replace(level2Element, "<div class=\"level2\" title=\"level2\">" + level2Element + "</div>");
                    } else if (element.textContent.trim().charAt(0) == "第" && element.textContent.trim().charAt(3) == "条") {
                        const level2Element = /.*第.*\d.*条\s/.exec(element.outerHTML);
                        element.outerHTML = element.outerHTML.replace(level2Element, "<div class=\"level2\" title=\"level2\">" + level2Element + "</div>");
                    }
                }
