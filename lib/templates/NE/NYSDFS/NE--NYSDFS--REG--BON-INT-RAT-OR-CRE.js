    ...
    try {
        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                // # root:info #
                // #############
                const rootTitle = "Bonus Interest Rate Or Credit Provisions In Deferred Annuity Contracts";
                wrapElementLevel1(document, rootTitle);

                const dates = document.querySelectorAll(".clnumdate");
                const regex = /^(January|February|March|April|May|June|July|August|September|October|November|December) \d{1,2} *,( |  )\d{4}/g; // customized match to handle non-standard date
                for (const date of dates) {
                    if (date.textContent.match(regex)) {
                        const issueDate = new Date(date.textContent.replace(" ", "")); // customized match to handle non-standard date
