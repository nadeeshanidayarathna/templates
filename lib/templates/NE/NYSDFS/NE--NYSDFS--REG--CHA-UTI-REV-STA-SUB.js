    ...
    try {
        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                // # root:info #
                // #############
                const level1Element = document.querySelectorAll(".ctorer")[1];
                wrapElement(document, level1Element, "level1");
                wrapElementDate(document, "issue-date", "2015-03-30" + "T00:00:00");
                // ################
                // # content:info #
                // ################
                ...
                wrapElement(document, content, "ease-content");
                 ...
                 const level2Element = document.querySelectorAll(".underb");
                 ...
                 for(const contentChild of level3Element){
                    if(contentChild.textContent.charAt(1) == "."){
                         wrapElement(document, contentChild, "level3");
                    }
                 }

                 
                 const allPTags = Array.from(document.querySelectorAll("p"))
                 const footnoteElements= allPTags.slice(allPTags.length-6,allPTags.length-1);
                 for(const contentChild of footnoteElements){
                    if(contentChild.localName == "p"&& contentChild.textContent.charAt(1)==" "){
