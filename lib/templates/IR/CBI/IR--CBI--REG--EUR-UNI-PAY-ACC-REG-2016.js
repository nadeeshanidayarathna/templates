    ...
    try {
        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                // # root:info #
                // #############
                const level1Element = document.querySelectorAll(".act-content>table>tbody>tr")[39];
                wrapElement(document, level1Element, "level1");
                wrapElementDate(document, "issue-date", "2016-09-16" + "T00:00:00");
                // ################e
                // # content:info #
                // ################
                ...
                Array.prototype.forEach.call(Array.from(document.querySelectorAll(".act-content>table>tbody>tr")).slice(0,38), function (node) { node.remove(); });



                const level2Elements = document.querySelectorAll("p");
                for (i=0;i<level2Elements.length;i++) {
                    if (level2Elements[i].textContent.startsWith("\nPART")) {
                ...
                for (i=0;i<level3Elements.length;i++){
                    if(level3Elements[i].textContent.match(regex)){
                 ...
                 const footnoteElements = forFootnoteElements.slice(forFootnoteElements.length-3,forFootnoteElements.length);
