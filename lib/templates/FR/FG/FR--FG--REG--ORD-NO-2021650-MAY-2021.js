    ...
    try {
        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                wrapElementDate(document, "issue-date", "2021-05-28" + "T00:00:00");
                ...
                const level2Elements = document.querySelectorAll('h2, h3');
                for (const level2Element of level2Elements) {
                    if (/^Titre/.test(level2Element.textContent.trim())) {
                        wrapElement(document, level2Element, "level2");
                    } else if (/^Chapitre/.test(level2Element.textContent.trim())) {
                        wrapElement(document, level2Element, "level3");
                    } else if (/^Section/.test(level2Element.textContent.trim())) {
                        wrapElement(document, level2Element, "level4");
                    }
                }

                const level3Element = document.querySelectorAll('article.js-child .name-article');
                wrapElements(document, level3Element, "level5");
