    ...
    try {
        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                wrapElementDate(document, "issue-date", "2013-11-29" + "T00:00:00");
                ...
                for (const elements of level2Element) {
                    if (elements.textContent.trim().match(/^\d+\.\s|^ANNEX/)) {
                        wrapElement(document, elements, "level2");
                    } else if (elements.textContent.trim().match(/^\d+\.\d+/)) {
                        wrapElement(document, elements.childNodes[0], "level3");
                    }
                }
                ...
                for (const elements of level3Element) {
                    if (elements.textContent.trim().match(/^\d+\.\d+/)) {
                        if (elements.childNodes[0].textContent.trim().match(/^\d+\.\d+$/)) {
                            wrapElement(document, elements.childNodes[0], "level3");
                        }
                    }
                }
