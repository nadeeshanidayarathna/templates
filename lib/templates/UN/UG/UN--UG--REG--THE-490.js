...
const scraper = async function download(url, sp, path) {
    try {
...

        {
            ...
            // 2.Identify
            // 3.Collect
            await page.waitForSelector("#viewLegContents");
            ...
            var tags = await page.evaluate(function process(url) {
...

                {
                    ...
                    for (const element of elements[0].childNodes) {
                        if (element.className == 'LegPartFirst' || element.className == 'LegPart') {
                        ...
                        } else if ((element.className == 'LegChapterFirst' || element.className == 'LegChapter') && element.textContent.startsWith('CHAPTER')) {
                            try { tags.push("<div class='level3' title='level3'>" + element.childNodes[0].outerHTML + " " + element.childNodes[1].outerHTML + "</div>"); nextLevel = 4; } catch (e) { }
                        ...
                        } else if (element.className == 'LegP1GroupTitleFirst' || element.className == 'LegP1GroupTitle') {
                            try { tags.push("<div class='level" + nextLevel + "' title='level" + nextLevel + "'>" + element.outerHTML + "</div>"); } catch (e) { }
                        } else if (element.className == 'LegChapterFirst' && !element.textContent.startsWith('CHAPTER')) {
                            try { tags.push("<div>" + element.childNodes[0].outerHTML + " " + element.childNodes[1].outerHTML + "</div>"); } catch (e) { }
                        } else if (element.className == 'LegPblockFirst' || element.className == 'LegPblock') {
                            try { tags.push("<div>" + element.childNodes[0].outerHTML + " " + element.childNodes[1].outerHTML + "</div>"); } catch (e) { }
                        } else if (element.className == 'LegAnnotations') {
                            for (const childs of element.childNodes) {
                                if (childs.textContent.startsWith('F')) {
