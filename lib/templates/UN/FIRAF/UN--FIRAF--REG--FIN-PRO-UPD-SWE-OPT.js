const base = require("../../../common/base");

const scraper = async function download(url, sp, path) {
    try {
        console.log("callings scraper url:" + url + " sp:" + sp + " path:" + path);
        const browser = await base.puppeteer().launch({
            headless: false,
            devtools: false
        });
        {
            // 1.Download
            console.log("getting url:" + url);
            const page = await browser.newPage();
            await base.downloadPage(page, url, sp, path);

            // 2.Identify
            // 3.Wrap
            await page.waitForSelector("body");
            await page.evaluate(function process() {
                // #############
                // # root:info #
                // #############
                const level1Element = document.querySelector('h1');
                wrapElement(document, level1Element, "level1");

                // ################
                // # content:info #
                // ################
                const contentElement = document.querySelector('.main-content');
                wrapElement(document, contentElement, "ease-content");

                const level2Element = document.querySelectorAll('h3');
                wrapElements(document, level2Element, "level2");

                const level3Element = document.querySelectorAll('strong');
                wrapElements(document, level3Element, "level3");

                const alterTexts = document.querySelectorAll("li");
                for (const elements of alterTexts) {
                    let texts = elements.innerHTML;
                    elements.outerHTML = elements.outerHTML.replace(texts, "<span> ● </span>" + texts);
                }

                const alterBullets = document.querySelectorAll("ul > li > ul > li");
                for (const elements of alterBullets) {
                    let texts = elements.innerHTML;
                    elements.outerHTML = elements.outerHTML.replace(texts, "<span> ○ </span>" + texts);
                }

                const alterSubbullets = document.querySelectorAll("ul > li > ul > li > ul > li");
                for (const elements of alterSubbullets) {
                    let texts = elements.innerHTML;
                    elements.outerHTML = elements.outerHTML.replace(texts, "<span> ▪ </span>" + texts);
                }



                // removing unwanted content from ease-content
                Array.prototype.forEach.call(document.querySelectorAll(" .gray-highlight"), function (node) { node.remove(); });

                
                
                return Promise.resolve();
            });
            // 4.Write
            await base.writePage(page, url, sp, path, true, true, false);
            await page.close();
        }
        await base.test().runPageTest(browser, url, sp, path);
        await browser.close();
    } catch (e) {
        console.log(e);
        console.log("Failed to scrape!!!");
        process.exit(1);
    }
}
module.exports = scraper;