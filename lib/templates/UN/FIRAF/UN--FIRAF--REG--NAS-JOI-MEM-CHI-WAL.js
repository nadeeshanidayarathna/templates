    ...
    try {
        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                const dates = document.querySelectorAll("main p");
                for (const elements of dates) {
                    if (elements.textContent.trim().match(/^DATE:/)) {
                        const issueDates = /[A-Z]+\s\d+,\s\d{4}/.exec(elements.textContent.trim());
                ...
                const level2Element = document.querySelectorAll("main .list b");
