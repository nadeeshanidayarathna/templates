  ...
  try {
    ...
    });
    {
      ...
      await page.evaluate(function process() {
        ...
        const level2Element = document.querySelectorAll(".field__item >p:nth-of-type(6),.field__item >p:nth-of-type(9)");
        wrapElements(document, level2Element, "level2");

        // removing unwanted content from ease-content
        Array.prototype.forEach.call(
          document.querySelectorAll(".view-tags__block, .banner, .region.region-header, .group-breadcrumb, .usa-footer.usa-footer--big, .usa-header.usa-header--extended.grid-container, .usa-skipnav, .view-header, .views-field.views-field-title, .usa-sr-only"),
          function (node) {
            node.remove();
          }
        );
