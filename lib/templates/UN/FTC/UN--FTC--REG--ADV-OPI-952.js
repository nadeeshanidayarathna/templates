        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                const level1Element = document.querySelector('h1');
                 ...
                 const issueDates=document.querySelectorAll(".field__item >p");
                 const IssueDatesArray = Array.from(issueDates);
                 const textArray = [];
                 IssueDatesArray.forEach(item => {
                    textArray.push(item.outerText);
                 })
                 console.log(textArray);
                 console.log(textArray.findIndex(item => item.includes('Date:')));
                 const index = textArray.findIndex(item => item.includes('Date:'));
                 console.log(typeof index);
                 console.log(issueDates[index]);
                 const firstdate = issueDates[index].firstChild;
