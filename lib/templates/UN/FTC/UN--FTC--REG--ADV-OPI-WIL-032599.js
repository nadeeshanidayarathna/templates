  ...
  try {
    ...
    });
    {
      ...
      await page.evaluate(function process() {
        ...
        const level2Element = document.querySelectorAll(
          ".field__item >p:nth-of-type(3)"
        );
        wrapElements(document, level2Element, "level2");

        const level3Element = document.querySelectorAll("p > em");
        wrapElements(document, level3Element, "level3");
 
        const footnoteElement = document.querySelectorAll("p");
        for (const child of footnoteElement) {
          if (child.innerHTML.includes('id="N_')) {
