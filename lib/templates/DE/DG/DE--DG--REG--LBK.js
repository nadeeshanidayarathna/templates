    ...
    try {
        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                // # root:info #
                // #############
                const level1Element = document.querySelectorAll('.PARA  FONT  P ')[0];
                ...
                const footnoteElement = document.querySelector("#NOTER");
                wrapElement(document, footnoteElement, "footnote");

                const ahreftags = document.querySelectorAll(".PARA a")[0];
                        const fullText = ahreftags.getAttribute("href");
                        console.log("fulltext : "+JSON.stringify(fullText))
                        let check = (fullText.indexOf("#"))
                        let part2 = fullText.substring(check, fullText.length);
                        ahreftags.outerHTML = ahreftags.outerHTML.replace(/href=\".*?\"/i, `href=\"${part2}\"`);                   
