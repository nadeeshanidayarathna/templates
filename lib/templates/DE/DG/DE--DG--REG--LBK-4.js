        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                for(const element of elements){
                    if(element.className == "Bog"){
                        let arrList = [];
                        arrList.push(element);
                        arrList.push(element.nextElementSibling);
                        wrapElements(document, arrList, "level2", group = "true");
                        levelNo = 3;
                    } else if(element.className == "Bilag"){
                        wrapElement(document, element, "level2");
                    } else if(element.className == "Afsnit"){
                        let arrList = [];
                        arrList.push(element);
                        if(element.nextElementSibling.className == "AfsnitOverskrift"){
                            arrList.push(element.nextElementSibling);
                        }                        
                        wrapElements(document, arrList, "level3", group = "true");
                        levelNo = 4;
                    } else if(element.className == "Kapitel"){
                        let arrList = [];
                        arrList.push(element);
                        if(element.nextElementSibling.className == "KapitelOverskrift2"){
                            arrList.push(element.nextElementSibling);
                        }  
                        wrapElements(document, arrList, "level"+levelNo, group = "true");
                        testLvl = levelNo + 1;
                    } else if(element.className == "Paragraf" && element.id != "id1c3bbbdf-05af-452a-b7ca-73c992de88e3"){
                        for(const childs of element.childNodes){
                            if(childs.nodeName == "SPAN"){
                                if(childs.className == "ParagrafNr"){
                                    wrapElement(document, childs, "level"+testLvl);
                ...
                Array.prototype.forEach.call(document.querySelectorAll(".col-5, .row > .col-md-6 > .py-2, .py-2"), function (node) { node.remove(); });
