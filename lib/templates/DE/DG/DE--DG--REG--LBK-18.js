        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                for(const element of elements){
                    if(element.className == "Kapitelnummer"){
                        let arrList = [];
                        arrList.push(element);
                        arrList.push(element.nextElementSibling);
                        wrapElements(document, arrList, "level2", group = true);
                    } else if(element.className == "Paragraftekst"){
                        for(const childs of element.childNodes){
                            if(childs.nodeName == "B"){
                                let arrList = [];
                                arrList.push(childs);
                                if(childs.nextSibling.nextSibling != null){
                                    arrList.push(childs.nextSibling.nextSibling);
                                }
                ...
                const footnoteElement = document.querySelectorAll(".PARA .Slutnotetekst");
                for(const element of footnoteElement){
                    if(element.firstElementChild.nodeName == "A"){
                        wrapElement(document, element, "footnote");
                    }
                }
  
                const ahreftags = document.querySelectorAll(".Paragraftekst a");
                ...
                Array.prototype.forEach.call(document.querySelectorAll(".col-5, .row > .col-md-6 > .my-2, .row > .col-md-6 > .py-2, .py-2, .paragraph-search-tab"), function (node) { node.remove(); });
