    ...
    try {
        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                const level2Bilag = document.querySelector('.Bilagstitel');
                wrapElement(document, level2Bilag, "level2");  
                
                const level3Bilag = document.querySelectorAll('.Tekstoverskrift');
                wrapElements(document, level3Bilag, "level3");  

                const footnoteElement = document.querySelectorAll(".Slutnotetekst");
                wrapElements(document, footnoteElement, "footnote");

                const ahreftags = document.querySelectorAll(".Titel a");
                ...
                const ahreftagsz = document.querySelectorAll(".Paragraftekst a");
                for(const ahreftag of ahreftagsz){   
                        const fullText = ahreftag.getAttribute("href");
                        let check = (fullText.indexOf("#"))
                        let part2 = fullText.substring(check, fullText.length);
                        console.log("part 2 :"+part2);
                ...
                const ahreftagsN = document.querySelectorAll(".Nummer A");
                for(const ahreftag of ahreftagsN){   
                        const fullText = ahreftag.getAttribute("href");
                        let check = (fullText.indexOf("#"))
                        let part2 = fullText.substring(check, fullText.length);
                        console.log("part 2 :"+part2);
