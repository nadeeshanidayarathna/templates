    ...
    try {
        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                const level1Element = "GUIDELINES ON STANDARDS OF CONDUCT FOR MARKETING AND DISTRIBUTION ACTIVITIES BY FINANCIAL INSTITUTIONS";
                ...
                for(const element of level3Elements){
                    if(element.textContent.match(/^\d{1,2}\s{1,}/) && element.parentElement.tagName != "TD"){
                        const level3ElementText = /^\d{1,2}/.exec(element.textContent);
                        element.outerHTML = element.outerHTML.replace(level3ElementText, "<div title=\"level3\" class=\"level3\">" + level3ElementText + "</div>");
                    }
                    else if(element.textContent.match(/^\d{1,2}[.]\d{1,2}\s{1,}/) && element.parentElement.tagName != "TD"){
