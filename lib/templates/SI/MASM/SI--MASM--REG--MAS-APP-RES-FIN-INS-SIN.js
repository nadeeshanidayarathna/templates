    ...
    try {
        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                wrapElementDate(document, "issue-date", "2017-08-23" + "T00:00:00");
                ...
                for (const element of elements) {
                    if (element.tagName == 'H2') {
                        ...
                        let title = /^\d+\.\d+/.exec(element.textContent.trim());
                        let texts = element.innerHTML.replace(title, "");
                        element.outerHTML = element.outerHTML.replace(element.innerHTML, `<div class=\'level${nextLevel}\' title=\'level${nextLevel}\'>${title}</div><div>${texts}</div>`);
                    }
                }
