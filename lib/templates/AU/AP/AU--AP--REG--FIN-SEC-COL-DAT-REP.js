    ...
    try {
        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                // const level2Element = document.querySelectorAll('.WordSection2 > p.MsoNormal > b > span');
                // let texts = level2Element[1].innerHTML;
                // let texts2 = level2Element[0].innerHTML;
                // level2Element[1].outerHTML = level2Element[1].outerHTML.replace(level2Element[1].innerHTML, "");
                // level2Element[0].outerHTML = level2Element[0].outerHTML.replace(level2Element[0].innerHTML, "<div class=\"level2\" title4=\"level2\">" + texts2 + " " + texts + "</div>");

                const level2Element2 = document.querySelectorAll('.WordSection1 > p.MsoNormal > b > span');
                for (const elements of level2Element2) {
                    if ((elements.style.fontSize == '14pt' || elements.style.fontSize == '16pt') && elements.textContent.trim() != "") {
                        wrapElement(document, elements, "level2");
                    } else if (elements.style.fontSize == '12pt' && elements.textContent.trim() != "") {
                        wrapElement(document, elements, "level3");
                    }
                }
