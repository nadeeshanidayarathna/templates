const { group } = require("yargs");
const base = require("../../../common/base");

const scraper = async function download(url, sp, path) {
    try {
        console.log("callings scraper url:" + url + " sp:" + sp + " path:" + path);
        const browser = await base.puppeteer().launch({
            headless: false,
            devtools: false
        });
        {
            // 1.Download
            console.log("getting url:" + url);
            const page = await browser.newPage();
            await base.downloadPage(page, url, sp, path, null, 'utf16le');

            // 2.Identify
            // 3.Wrap
            await page.waitForSelector(".doc-content");
            await page.evaluate(function process() {
                // #############
                // # root:info #
                // #############

                const rootTitle = "SPG 511 Remuneration";
                wrapElementLevel1(document, rootTitle);

                wrapElementDate(document, "issue-date", "2013-11-01T00:00:00");

                // ################
                // # content:info #
                // ################

                const content = document.querySelector(".doc-content");
                wrapElement(document, content, "ease-content");

                const contentL2 = document.querySelector("h1");
                wrapElement(document, contentL2, "level2");

                const contentL3 = document.querySelectorAll("h2");
                wrapElements(document, contentL3, "level3");

                const contentL4 = document.querySelectorAll("h3");
                wrapElements(document, contentL4, "level4");

                const footnoteElements = document.querySelectorAll(".c244");
                wrapElements(document, footnoteElements, "footnote");

                return Promise.resolve();
            });
            // 4.Write
            await base.writePage(page, url, sp, path, true, true, false);
            await page.close();
        }
        await base.test().runPageTest(browser, url, sp, path);
        await browser.close();
    } catch (e) {
        console.log(e);
        console.log("Failed to scrape!!!");
        process.exit(1);
    }
}
module.exports = scraper;