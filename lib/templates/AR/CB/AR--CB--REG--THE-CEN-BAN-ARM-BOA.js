    ...
    try {
...

        {
            ...
            var tags = await page.evaluate(function process(url) {
                ...
                const effectiveDate = document.querySelectorAll('#EffectiveDate')[0];
                try { tags.push("<div class='effective-date' title='effective-date'>" + (new Date(effectiveDate.textContent).getFullYear()) + "-" + ("0" + (new Date(effectiveDate.textContent).getMonth() + 1)).slice(-2) + "-" + ("0" + (new Date(effectiveDate.textContent).getDate())).slice(-2) + "T00:00:00</div>"); } catch (e) { }
                ...
                const rootElements = document.querySelectorAll("table[bgcolor='#fccf00']");
                AddTransform(rootElements[0]);
        ...
        await test.runTest(browser, originalHtmlPath, originalTextPath, buildHtmlPath, buildTextPath, ".documentBody", ["#RelationsTable","#LinksTable","#Title"], "body", [".level1"]);
        await browser.close();
    } catch (e) {
        console.log(e);
