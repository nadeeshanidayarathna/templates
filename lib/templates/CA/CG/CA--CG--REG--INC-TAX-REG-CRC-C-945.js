    ...
    try {
...

        {
            ...
            var tags = await page.evaluate(function process(url) {
                ...
                try { tags.push("<div class='issue-date' title='issue-date'>" + "2021-08-12" + "T00:00:00</div>"); } catch (e) { }
                ...
                Array.prototype.forEach.call(document.querySelectorAll("dt .DefinedTerm"), function (node) { node.remove(); });
                Array.prototype.forEach.call(document.getElementsByTagName("li"), function (node) { node.style.listStyleType = 'none'; });
                Array.prototype.forEach.call(document.getElementsByTagName("a"), function (node) { const href = node.getAttribute("href"); if (href != null && href.length > 0) { if (href.charAt(0) == "#") { node.setAttribute("href", url + href); } else if (href.charAt(0) == "/" || href.charAt(0) == "s") { node.setAttribute("href", (new URL(url)).origin + href); } } });
                ...
                for (const sectionChild of sectionChilds) {
                    ...
                    if (sectionChild.tagName == "H2") {
                        ...
                        for (const schedule of schedules) {
                            ...
                            for (const scheduleChild of scheduleChilds) {
                                ...
                                } else if (scheduleChild.tagName == "H3") {
                                    if (scheduleChild.childNodes.length > 1) {
                                        try { tags.push("<div class='level4' title='level4'>" + scheduleChild.childNodes[0].outerHTML + " " + scheduleChild.childNodes[1].outerHTML + "</div>"); } catch (e) { }
                                    } else {
                                        try { tags.push("<div class='level4' title='level4'>" + scheduleChild.outerHTML + "</div>"); } catch (e) { }
                                    }
                                } else if (scheduleChild.tagName == 'DIV' && scheduleChild.className == "HistoricalNote") {
                                    for (const childs of scheduleChild.childNodes) {
                                        let i = 0;
                                        for (const child of childs.childNodes) {
                                            if (i < (childs.childNodes.length - 1)) {
                                                try { tags.push("<div>" + child.outerHTML + "<div class='pseodo-element'>;</div></div>"); } catch (e) { }
                                            } else {
                                                try { tags.push("<div>" + child.outerHTML + "<div class='pseodo-element'>.</div></div>"); } catch (e) { }
                                            } i++;
                                        }
                    ...
                    } else if ((h2MultiTags == false && previousTagName == "H2" && sectionChild.tagName == "P") || (h2MultiTags == false && previousPreviousTagName == "H2" && previousTagName == "H3" && sectionChild.tagName == "P")) {
                        if (sectionChild.textContent != "") {
                            try { tags.push("<div class='level3' title='level3'>" + sectionChild.outerHTML + "</div>"); } catch (e) { }
                        }
                    } else if ((sectionChild.tagName == "H3") && sectionChild.textContent != "") {
                        if (sectionChild.childNodes.length > 1) {
                            try { tags.push("<div class='level3' title='level3'>" + sectionChild.childNodes[0].outerHTML + " " + sectionChild.childNodes[1].outerHTML + "</div>"); } catch (e) { }
                        } else {
                            try { tags.push("<div class='level3' title='level3'>" + sectionChild.outerHTML + "</div>"); } catch (e) { }
                        }
                    } else if (sectionChild.tagName == "H4") {
                        try { tags.push("<div class='level4' title='level4'>" + sectionChild.outerHTML + "</div>"); } catch (e) { }
                    } else if (sectionChild.tagName == "H5" && sectionChild.textContent != "") {
                        if (sectionChild.childNodes.length > 1) {
                            try { tags.push("<div class='level4' title='level4'>" + sectionChild.childNodes[0].outerHTML + "</div>"); sectionChild.childNodes[0].remove(); } catch (e) { }
                            AddTransform(sectionChild);
                        } else {
                            try { tags.push("<div class='level4' title='level4'>" + sectionChild.outerHTML + "</div>"); } catch (e) { }
                        }
                    } else if (sectionChild.tagName == 'UL') {
                        for (const childs of sectionChild.childNodes) {
                            for (const child of childs.childNodes) {
                                if (child.tagName == "H5") {
                    ...
                    } else if (sectionChild.tagName == 'DIV' && sectionChild.className == "HistoricalNote") {
                        for (const childs of sectionChild.childNodes) {
                ...
                for (const contentSection2 of contentSection2s) {
                    ...
                    for (const contentSection2Child of contentSection2Childs) {
                        ...
                        } else if (contentSection2Child.tagName == 'UL') {
                            const element1 = contentSection2Child.querySelectorAll('p.nifrpCitation')[0];
                            AddTransform(element1);

                            const element2s = contentSection2Child.querySelectorAll('ul.Section.ProvisionList > li');
                            let a = 0;
                            for (const element2 of element2s) {
                                if (a != 1) {
                                    for (const childs of element2.childNodes) {
                                        if (childs.tagName == 'P' && childs.childNodes.length > 3) {
        ...
        await test.runTest(browser, originalHtmlPath, originalTextPath, buildHtmlPath, buildTextPath, "main", [".legisHeader", ".FCSelector", "#right-panel", ".wb-invisible", "dt .DefinedTerm", ".mfp-hide", "#wb-dtmd", ".PITLink"], "body", [".pseodo-element"]);
        await browser.close();
    } catch (e) {
        console.log(e);
