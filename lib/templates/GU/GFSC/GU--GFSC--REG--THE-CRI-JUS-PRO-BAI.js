    ...
    try {
        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                const rootTitle = document.querySelectorAll(".c55")[0];
                ...
                for(const child of level2Element){
                    if(child.textContent.match(/^PART /) || child.textContent.match(/^\[SCHEDULE/) || child.textContent.match(/^FINANCIAL /)){
                        wrapElement(document, child, "level2");
                    }
                }
                

                const level3Element = document.querySelectorAll("H6");
                wrapElements(document, level3Element, "level3");

                const level4Element = document.querySelectorAll(".MsoSubtitle");
                wrapElements(document, level4Element, "level4");

                const footnoteElement = document.querySelectorAll(".c18");
