...
const scraper = async function download(url, sp, path) {
    try {
        ...
        });
        {
            ...
            // 2.Identify
            // 3.Wrap
            await page.waitForSelector(".fulltext > div:nth-of-type(2)");
            await page.evaluate(function process() {
                // #############
                // # root:info #
                // #############
                const level1Element = document.querySelectorAll(".toanvancontent > p[align='center']");
                wrapElements(document, level1Element, "level1", group = true);
                wrapElementDate(document, "issue-date", "2022-06-29" + "T00:00:00");
                wrapElementDate(document, "effective-date", "2022-08-15" + "T00:00:00");

                // ################
                // # content:info #
                // ################
                const contentElement = document.querySelector('.fulltext > div:nth-of-type(2)');
                wrapElement(document, contentElement, "ease-content");

                const level2Elements = document.querySelectorAll(".toanvancontent strong");
                for (const level2Element of level2Elements) {
                    if (level2Element.align != 'center' && level2Element.childNodes[1]){
                        wrapElement(document, level2Element, "level2");
                    }
                }
                console.log(level2Elements.length)
