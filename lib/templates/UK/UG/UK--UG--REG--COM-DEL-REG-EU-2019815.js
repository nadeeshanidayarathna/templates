    ...
    try {
...

        {
            ...
            var tags = await page.evaluate(function process(url) {
                ...
                try { tags.push("<div class='issue-date' title='issue-date'>" + "2018-12-17" +"T00:00:00</div>"); } catch (e) { }
                try { tags.push("</div>"); } catch (e) { }

                // remove hidden & unwanted elements + correcting links for img and a tags
                Array.prototype.forEach.call(document.querySelectorAll(".changed-by"), function (node) { node.remove(); });
        ...
        await test.runTest(browser, originalHtmlPath, originalTextPath, buildHtmlPath, buildTextPath, ".handbook-content", [".changed-by"], "body", [".level1"]);
