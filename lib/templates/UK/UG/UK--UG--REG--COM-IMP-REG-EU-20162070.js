                ...
                function AddTransform(node) {
                    if (node != undefined) {
                        if (node.nodeType != Node.TEXT_NODE) {
                            const missingImageUrl = "https://www.handbook.fca.org.uk/publication/techstandards/graphic/CRD_2016_2070_Annex_IV_inline_1_formula.png";
                            const missingImageNode = node.querySelectorAll("img[src='" + missingImageUrl + "']");
                            if (missingImageNode.length == 1) {
                                if (!ImageExists(missingImageUrl)) {
                                    console.log("removing missing image");
                                    missingImageNode[0].remove();
                                }
                            }
                        }
                                                ...
                                                for (const childLevel3 of childLevel2.childNodes) {
                                                    if (childLevel3.className == 'section-content-table') {
                                                        ...
                                                        for (const childLevel4 of childLevel3.childNodes) {
                                                            if (childLevel4.tagName == 'P' && (i == 1 || childLevel4.outerText.startsWith('PART'))) {
                                                                try { tags.push("<div class='level3' title='level3'>" + childLevel4.textContent + "</div>"); childLevel4.remove(); } catch (e) { }
                                                            } else if (childLevel4.tagName == 'P' && childLevel4.outerText.startsWith('Section')) {
                                                                try { tags.push("<div class='level4' title='level4'>" + childLevel4.textContent + "</div>"); childLevel4.remove(); } catch (e) { }
                                                            } else {
                                                                AddTransform(childLevel4);
                                                            } i++;
                                                        }
                                                    } else {
                                                        AddTransform(childLevel3);
                                                    }
                                                }
                                            } else {
                                                AddTransform(childLevel2);
                                            }
                                        }
                                    } else {
                                        AddTransform(child);
