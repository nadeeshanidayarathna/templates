    ...
    try {
        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                wrapElementLevel1(document, level1Elements[0].textContent + " " + level1Elements[1].textContent + " " + level1Elements[2].textContent + " " + level1Elements[3].textContent);
                wrapElementDate(document, "issue-date", "2019-04-17" + "T00:00:00");
                ...
                for (const element of level2Elements) {
                    if (element.className == 'ti-section-1') {
                        if (element.textContent.trim().match(/^T.*/)) {
                            let elements = [element, element.nextElementSibling]; 
                            wrapElements(document, elements, "level2", group = true);
                            nextLevel = 3;
                        } else if (element.textContent.trim().match(/^C.*/)) {
                            let elements = [element, element.nextElementSibling]; 
                            wrapElements(document, elements, "level3", group = true);
                            nextLevel = 4;
                        } else {
                            let elements = [element, element.nextElementSibling]; 
                            wrapElements(document, elements, "level4", group = true);
                            nextLevel = 5;
                        }
                    } else {
