    ...
    try {
        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                wrapElementDate(document, "issue-date", "1996-09-10" + "T00:00:00");
                // ################
                // # content:info #
                // ################
                ...
                const regexL2 = /^(\d\.\s)/i;
                const regexL2space = /^(\s\d\.\s)/i;
                const regexL3 = /^([ivx]+\)\s)/i;
                const regexL3space = /^(\s[ivx]+\)\s)/i;
                for (const contentChild of level2Elements) {
                    ...
                    } else if (contentChild.textContent.match(regexL2space)) {

                        level2Text = contentChild.textContent.match(regexL2space)[1];
                        contentChild.outerHTML = contentChild.outerHTML.replace(level2Text, "<div title=\"level2\" class=\"level2\">" + level2Text + "</div>");
                    ...
                    } else if (contentChild.textContent.match(regexL3space)) {

                        level3Text = contentChild.textContent.match(regexL3space)[1];
                        contentChild.outerHTML = contentChild.outerHTML.replace(level3Text, "<div title=\"level3\" class=\"level3\">" + level3Text + "</div>");
