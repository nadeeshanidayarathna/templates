    ...
    try {
        ...
        });
        {
            ...
            await page.evaluate(function process() {
                ...
                // # content:info #
                // ################
                const contentElement = document.querySelector(".c9");
                wrapElement(document, contentElement, "ease-content");

                const level2Element = document.querySelector("h2");
                wrapElement(document, level2Element, "level2");

                const level3Elements = document.querySelectorAll(".c0 > .c3");
                const regex = /(^([(]((?!I)[A-Z])[)]\s))/;
                for (const contentChild of level3Elements) {
                    if (contentChild.textContent.match(regex)) {
                            level3Text = contentChild.textContent.slice(0,3);
