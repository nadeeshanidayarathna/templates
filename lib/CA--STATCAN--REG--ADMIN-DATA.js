...
const scraper = async function download(url, sp, path) {
    try {
                        ...
                        for (const innerElement of innerElements) {
                            if (innerElement.tagName == "P") {
                                ...
                                for (const listElement of listElements) {
                                    index++;
                                    try { tags.push("<span class='level3' title='level3'>" + index + ".</span>"); } catch (e) { }
        ...
        await base.runTest(browser, originalHtmlPath, originalTextPath, buildHtmlPath, buildTextPath, ".region-laurier-first", [".list-unstyled.mrgn-lft-md"]);
        await browser.close();
    } catch (e) {
        console.log(e);
