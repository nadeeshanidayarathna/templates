...
const scraper = async function download(url, sp, path) {
    try {
                        ...
                        for (const element of elements) {
                            if (element.className == 'LegClearFix LegPrelims') {
                                ...
                                for (const child of childs) {
                                    ...
                                    else if (child.className != "LegAnchorID") {
                                        AddTransform(child);
                                    }
                                }
                            }
                            ...
                            else if (element.className == 'LegPartFirst' || element.className == 'LegPart') {
                            ...
                            else if (element.tagName == 'H3' && (element.className == 'LegChapterFirst' || element.className == 'LegChapter')) {
                            ...
                            } else if (element.tagName == 'H5' && (element.className == 'LegChapterFirst' || element.className == 'LegChapter')) {
                                try { tags.push("<div class='level3' title='level3'>" + element.childNodes[0].outerHTML + " " + element.childNodes[1].outerHTML + "</div>"); nextLevel=4;} catch (e) { }
                            } else if ((element.tagName == 'H5' || element.tagName == 'H6') && (element.className == 'LegPblockFirst' || element.className == 'LegPblock')) {
                            ...
                            else if (element.className != "LegAnchorID") {
                                if (element.className == "LegAnnotations") {
                                    try { tags.push("<div class='footnote' title='footnote'>" + element.outerHTML + "</div>"); element.remove(); } catch (e) { }
                                } else {
        ...
        await test.runTest(browser, originalHtmlPath, originalTextPath, buildHtmlPath, buildTextPath, "#layout2", [".contentSearch", "#breadCrumb", "#legSubNav", ".interface", "#tools", "#printOptions", "#changesOverTime", "#statusWarning", ".contentFooter", ".backToTop", ".LegExtentRestriction"], "body", []);
        await browser.close();
    } catch (e) {
        console.log(e);
