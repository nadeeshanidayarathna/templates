    ...
    try {
...

        {
            ...
            var tags = await page.evaluate(function process(url) {
                ...
                //transforms under the root
                const elementTransform = document.querySelectorAll(".system_details_box");
                AddTransform(elementTransform[0]);

                //starting select levels
                const elements = document.querySelectorAll(".system_terms_textbox > div"); //select whole text
                for (const element of elements[0].childNodes){ //looping
                    if (element.tagName == "H4" && element.className == "center"){
                        try { tags.push("<div class='level2' title='level2'>" + element.outerHTML + "</div>"); } catch (e) { } //add to level2
