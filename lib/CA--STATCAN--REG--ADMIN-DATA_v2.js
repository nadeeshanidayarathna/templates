    ...
    try {
        ...
        // ########################
        {
            ...
            var tags = await page.evaluate(async function process() {
                ...
                // simplify
                Simplify(tags, document);

                return Promise.resolve(tags);

                function Simplify(tags, node) {
                    const childNodes = node.childNodes;
                    for (childNode of childNodes) {
                        Simplify(tags, childNode);
                    }
                    if (node.nodeType == Node.TEXT_NODE) {
                        //tags.push(node.textContent);
                    } else if (node.nodeType == Node.ELEMENT_NODE) {
                        if ("SPAN" == node.nodeName && Array.from(node.classList).toString().indexOf("level") > -1) {
                            tags.push(node.outerHTML);
                            tags.push("<br>");
                        } else {
                            //tags.push(node.textContent);
                        }
