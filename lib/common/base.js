...
async function download(id, originalHtmlPath, metadataPath, page, url, params, encoding = "utf8") {
    if (fs.existsSync(originalHtmlPath)) {
        ...
        var metaData = await getMetaData(id, url, headers);
...
async function downloadPage(page, url, sp, path, params, encoding = "utf8") {
    ...
    if (fs.existsSync(originalHtmlPath)) {
        ...
        if (fs.existsSync(metadataPath)) {
            ...
            // creating default metadata file as its required by downstream services to process urls that has PDF content
            var metaData = await getMetaData(id, url, { "content-type": "text/html" });
            await fs.writeFileSync(metadataPath, JSON.stringify(metaData, null, 2));
        }
        const contentHtml = fs.readFileSync(originalHtmlPath, encoding);
        await page.setContent(contentHtml, { waitUntil: "domcontentloaded", timeout: 0 });
...
async function getMetaData(id, url, headers) {
    return {
        "uri": url,
        "cachedUri": id,
        "contentType": headers["content-type"],
        "cachedDateTimeOffset": new Date().toISOString().replace("Z", "+00:00"),
...
async function reLocateDOM(keepLevel1InEaseContent) {
    ...
    } else if (easeContentCount == 0) {
        throw "[VALIDATION]:ease-content not exists !!! please check the HTML content.";
    }
