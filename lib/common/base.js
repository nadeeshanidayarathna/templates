...
async function createFolder(url, sp, path) {
    const id = sha1(url);
    const outputPath = path + pathLib.sep + sp;
    const originalHtmlPath = outputPath + pathLib.sep + id + "_original.html";
    const buildHtmlPath = outputPath + pathLib.sep + id + ".html";
    const originalTextPath = outputPath + pathLib.sep + id + "_original.txt";
    const buildTextPath = outputPath + pathLib.sep + id + ".txt";
    const metadataPath = outputPath + pathLib.sep + id + "_metadata.json";
    if (!fs.existsSync(outputPath)) {
        fs.mkdirSync(outputPath);
    }
    return { originalHtmlPath, buildHtmlPath, originalTextPath, buildTextPath, metadataPath };
}
...
async function download(originalHtmlPath, metadataPath, page, url) {
    var loadedUrl;
    if (fs.existsSync(originalHtmlPath)) {
        ...
        var metaData = { url: url, loadedUrl: loadedUrl, contentType: headers["content-type"], contentLength: headers["content-length"], lastModified: headers["last-modified"], eTag: headers["etag"] };
