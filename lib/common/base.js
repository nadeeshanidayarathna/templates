...
async function originalToText(browser, originalHtmlPath, originalTextPath, originalRootScope, removeSelectors) {
    {
        ...
        var tags = await page.evaluate(function process(originalRootScope, removeSelectors) {
            var tags = [];
        ...
        }, originalRootScope, removeSelectors);
        await write(originalTextPath, tags);
        await page.close();
    }
}

async function buildToText(browser, buildHtmlPath, buildTextPath) {
    {
        const page = await browser.newPage();
        ...
        var tags = await page.evaluate(function process() {
            ...
            ToText(tags, document.querySelectorAll("body")[0]);
            return Promise.resolve(tags);
        });
...
async function runTest(browser, originalHtmlPath, originalTextPath, buildHtmlPath, buildTextPath, originalRootScope, removeSelectors) {
    await originalToText(browser, originalHtmlPath, originalTextPath, originalRootScope, removeSelectors);
    await buildToText(browser, buildHtmlPath, buildTextPath);
}
