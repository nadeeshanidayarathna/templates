 ...
 * @deprecated Used only in spider-ease v1. Use spider-ease v2 for new SPs
 */
async function download(id, originalHtmlPath, metadataPath, page, url, params) {
    var loadedUrl;
...
async function downloadPage(page, url, sp, path, params) {
    const id = sha1(url);
    const outputPath = pathLib.join(path, sp);
    const originalHtmlPath = pathLib.join(outputPath, id + "_original.html");
    const metadataPath = pathLib.join(outputPath, id + ".json");
...
async function loadPage(page, url, sp, path) {
    const id = sha1(url);
    const outputPath = pathLib.join(path, sp);
    const htmlPath = pathLib.join(outputPath, id + "_annotated.html");
    if (!fs.existsSync(htmlPath)) {
        throw id + ".html file not found. makesure it exists in the outputPath:" + outputPath
    }
    console.log("using existing cache file");
...
async function reLocateDOM() {
    ...
    if (easeRootCount > 1) {
        throw "[VALIDATION]:multiple .ease-root exists !!! please check the HTML content.";
    } else if (easeRootCount == 1) {
        root = document.querySelectorAll(".ease-root")[0];
    } else {
        // creating ease-root and moving level1 inside that as a child
    ...
    if (root.parentNode == null || root.parentNode.nodeName.toLowerCase() != "body") {
        root.append(level1);
    }
    ...
    } else if (easeContentCount == 1) {
        // getting ease-content with not siblingfy levels inside
        content = document.querySelectorAll(".ease-content")[0];
    }
...
async function writePage(page, url, sp, path, executeWriteAnnotatedPage, executeCleanupPageParam) {
    console.log("calling writePage");
    if (executeWriteAnnotatedPage) {
        await writeAnnotatedPage(url, path, sp, page);
    }
    if (executeCleanupPageParam) {
        await cleanupPage(page, url, sp, path);
    }
    await reLocatePage(page, url, sp, path);
    console.log("ending writePage");
}

async function writeAnnotatedPage(url, path, sp, page) {
    console.log("calling writeAnnotatedPage");
    const id = sha1(url);
    const outputPath = pathLib.join(path, sp);
    const annotatedHtmlPath = pathLib.join(outputPath, id + "_annotated.html");
    var html = await page.content();
    await fs.writeFileSync(annotatedHtmlPath, html);
