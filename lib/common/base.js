...
async function createFolder(url, sp, path) {
    ...
    const metaDataPath = outputPath + "\\" + id + "_metadata.txt";
    if (!fs.existsSync(outputPath)) {
        fs.mkdirSync(outputPath);
    }
    return { downloadPath, buildPath, metaDataPath };
}
...
async function writeMetaData(metaDataPath, xPaths) {
    fs.unlink(metaDataPath, (err => { }));
    var fileWriter = fs.createWriteStream(metaDataPath, {
        flags: 'a'
    });
    xPaths.forEach(function (xPath, i) {
        if (i != 0) {
            fileWriter.write("\n");
        }
        fileWriter.write(xPath);
    });
}
...
async function annotate(path, page) {
    const html = await page.content();
