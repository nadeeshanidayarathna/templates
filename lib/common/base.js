...
async function createFolder(url, sp, path) {
    ...
    const annotatedPath = outputPath + "\\" + id + "_annotated.html";
    if (!fs.existsSync(outputPath)) {
        fs.mkdirSync(outputPath);
    }
    return { downloadPath, buildPath, annotatedPath };
}
...
async function annotate(annotatedPath, page) {
    const html = await page.content();
    await fs.writeFileSync(annotatedPath, html);;
}


function wrap(document, element) {
    const span = document.createElement("span");
    span.classList.add("level1");
