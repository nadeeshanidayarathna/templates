...
const test = require("../tests/UN--FDICF--REG--POL-3.test");
    ...
    try {
...

        {
            ...
            var tags = await page.evaluate(function process(url) {
                ...
                const issueDate = /\w+\s\d+,\s\d{4}/.exec(issueDates[2].textContent);
                ...
                try { tags.push("<div class='effective-date' title='effective-date'>" + "2021-09-23" + "T00:00:00</div>"); } catch (e) { } // TODO: remove the hardcode
                        ...
                        if (element.tagName == "P" && element.childElementCount != 0) {
                            ...
                            for (const child of childs) {
                                if (child.tagName == "STRONG") {
                                    ...
                                    if (a == 2) {
                                        AddTransform(child); child.remove();
                                    }
                                    else {
