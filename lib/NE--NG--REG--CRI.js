...

        {
            ...
            var tags = await page.evaluate(function process(url) {
                ...
                try { tags.push("<div class='content'>"); } catch (e) { }
                {
                    ...
                    const boekElements = mainContentElement2.querySelectorAll(".boek");
                    for (boekElement of boekElements) {
                        const headingBoek = boekElement.querySelectorAll(".article__header--law.boek > h4")[0];
                        try { tags.push("<div class='level2' title='level2'>" + headingBoek.innerHTML + "</div>"); headingBoek.remove(); } catch (e) { }
                        const titleElements = boekElement.querySelectorAll(".titeldeel");
                        for (titleElement of titleElements) {
                            const title = titleElement.querySelectorAll(".article__header--law.titeldeel")[0];
                            try { tags.push("<div class='level3' title='level3'>" + title.innerHTML + "</div>"); title.remove(); } catch (e) { }
