    ...
    try {
...

        {
            ...
            var tags = await page.evaluate(function process(url) {
                ...
                try { tags.push("<div class='level1' title='level1'>" + elements[0].innerHTML + "</div>"); } catch (e) { }
                ...
                try { tags.push("<div class='issue-date' title='issue-date'>" + (new Date(dateStr).getFullYear()) + "-" + (new Date(dateStr).getMonth() + 1) + "-" + (new Date(dateStr).getDate() > 9 ? "" + new Date(dateStr).getDate() : "0" + new Date(dateStr).getDate()) + "T00:00:00</div>"); } catch (e) { }
                ...
                Array.prototype.forEach.call(document.getElementsByTagName("img"), function (node) { const src = node.getAttribute("src"); if (src != null && src.length > 0) { if (src.charAt(0) == "/") { node.setAttribute("src", (new URL(url)).origin + src); } } });
                Array.prototype.forEach.call(document.getElementsByTagName("a"), function (node) { const href = node.getAttribute("href"); if (href != null && href.length > 0) { if (href.charAt(0) == "/") { node.setAttribute("href", (new URL(url)).origin + href); } } });
                            ...
                            if (element.className == "LegClearFix LegPrelims") {
                                ...
                                for (const child of childs) {
                                    if (Array.prototype.indexOf.call(childs, child) == 2) {
                                        try { tags.push("<div class='level2' title='level2'>" + child.innerHTML + "</div>"); } catch (e) { }
                                    }
                                    else {
                                        AddTransform(child);
                                    }
                                }
                            } else if (element.className == "LegSchedulesTitle" || element.className == "LegExpNoteTitle") {
                                ...
                                try { tags.push("<div class='level3' title='level3'>" + element.innerHTML + "</div>"); } catch (e) { }
                            } else if (element.tagName.toUpperCase() == "H3") {
                                try { tags.push("<div class='level4' title='level4'>" + element.innerHTML + "</div>"); } catch (e) { }
                            }
                            else {
                                const footNotes = element.querySelectorAll(".LegCommentaryItem");
                                for (footNote of footNotes) {
                                    if (footNote.getAttribute("id").indexOf("commentary-key-") == -1) {
                                        try { tags.push("<div class='footnote' title='footnote'>" + footNote.innerHTML + "</div>"); footNote.remove(); } catch (e) { }
                                    }
                                }
        ...
        await base.runTest(browser, originalHtmlPath, originalTextPath, buildHtmlPath, buildTextPath, "#pageTitle,#viewLegSnippet", [".LegExtentRestriction"], "body", []);
        await browser.close();
    } catch (e) {
        console.log(e);
