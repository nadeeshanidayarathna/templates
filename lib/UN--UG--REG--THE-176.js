...
const scraper = async function download(url, sp, path) {
    try {
                        ...
                        for (const element of elements) {
                            if (element.className == 'LegClearFix LegPrelims') {
                                ...
                                for (const child of childs) {
                                    ...
                                    else if (child.className != 'LegTitle') {
                                        AddTransform(child);
                                    }
                                }
                            }
        ...
        await base.runTest(browser, originalHtmlPath, originalTextPath, buildHtmlPath, buildTextPath, "#viewLegSnippet", ['h1.LegTitle'], "body", ['div.level1']);
        await browser.close();
    } catch (e) {
        console.log(e);
