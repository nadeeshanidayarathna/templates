...
const scraper = async function download(url, sp, path) {
    try {
...

        {
            ...
            // 2.Identify
            // 3.Collect
            await page.waitForSelector("#layout2");
                        ...
                        for (const element of elements) {
                            if (element.className == "LegClearFix LegPrelims") {
                                ...
                                for (const child of childs) {
                                    if (child.className == "LegSubject") {
                                        try { tags.push("<div class='level2' title='level2'>" + child.textContent + " " + child.nextElementSibling.textContent + "</div>"); child.remove(); child.nextElementSibling.remove(); } catch (e) { }
                                    }
                            ...
                            else if (element.className == "LegScheduleFirst" || element.className == "LegSchedule") {
                                try { tags.push("<div class='level3' title='level3'>" + element.childNodes[0].outerHTML + " " + element.childNodes[1].outerHTML + "</div>"); } catch (e) { }
                            }
