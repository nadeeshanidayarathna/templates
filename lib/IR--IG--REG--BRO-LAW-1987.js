...

        {
            ...
            var tags = await page.evaluate(function process(url) {
                ...
                for(const childElements of element.childNodes){
                    if(childElements.id == "ddd2"){
                        try { tags.push("<div class='level2' title='level2'>" + childElements.outerText+ "</div>"); } catch (e) { }
                    } else if(childElements.id == "ddd" && childElements.textContent.includes("المادة")){
                        try { tags.push("<div class='level3' title='level3'>" + childElements.outerText+ "</div>"); } catch (e) { }
