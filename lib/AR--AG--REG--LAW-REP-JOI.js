    ...
    try {
...

        {
            ...
            var tags = await page.evaluate(function process(url) {
                ...
                Array.prototype.forEach.call(document.querySelectorAll("a[name]"), function (node) { node.remove(); });
                ...
                try { tags.push("<div class='adopted-date' title='adopted-date'>" + adoptedDate[0].textContent + "</div>"); } catch (e) { }
                {
                    ...
                    for (const element of elements){
                        ...
                        for(const childs of element.childNodes){
                             if(childs.tagName == "B" && childs.childNodes[0].tagName == "I"){
                                try { tags.push("<div class='level4' title='level4'>" + childs.outerHTML + "</div>"); } catch (e) { }
                            }   else if(childs.tagName == "B" && childs.childNodes[0].tagName != "I" && element.parentElement.nodeName == "CENTER" ){
                                try { tags.push("<div class='level3' title='level3'>" + childs.textContent+ "<br>" +element.nextElementSibling.textContent+ "</div>"); } catch (e) { }
                            } else if(childs.tagName == "B" && childs.childNodes[0].tagName != "I" && element.parentElement.nodeName == "DIV"){
        ...
        await test.runTest(browser, originalHtmlPath, originalTextPath, buildHtmlPath, buildTextPath, ".content", ["table", ".blue_mid_b","li", ".footer_options"], "body", []);
        await browser.close();
    } catch (e) {
        console.log(e);
