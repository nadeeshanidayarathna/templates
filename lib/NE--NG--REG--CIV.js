...

        {
            ...
            var tags = await page.evaluate(function process(url) {
                ...
                try { tags.push("<div class='content'>"); } catch (e) { }
                {
                    ...
                    Array.prototype.forEach.call(mainContent.querySelectorAll(".wettekst"), function (node) { node.remove(); });

                    try { tags.push("<div>" + mainContent.innerHTML + "</div>"); } catch (e) { }
                    const subHeading = mainContentElement1.querySelectorAll(".article__header--law.boek")[0];
                    try { tags.push("<div class='level2' title='level2'>" + subHeading.innerHTML + "</div>"); subHeading.remove(); } catch (e) { }
                    ...
                    for (titleElement of titleElements) {
                        ...
                        try { tags.push("<div class='level3' title='level3'>" + titleHeading.innerHTML + "</div>"); titleHeading.remove(); } catch (e) { }
                        ...
                        if (afdelingElements.length > 0) {
                            for (afdelingElement of afdelingElements) {
                                ...
                                try { tags.push("<div class='level4' title='level4'>" + headingAfdeling.innerHTML + "</div>"); headingAfdeling.remove(); } catch (e) { }
                                AddTransforms(afdelingElement.querySelectorAll(".article__header--law.afdeling > p"));
                                const paragraafs = afdelingElement.querySelectorAll(".paragraaf");
                                if (paragraafs.length > 0) {
                                    for (paragraaf of paragraafs) {
                                        ...
                                        try { tags.push("<div class='level5' title='level5'>" + paraHeading.innerHTML + "</div>"); paraHeading.remove(); } catch (e) { }
                                        ...
                                        for (articleElement of articleElements) {
                                            ...
                                            try { tags.push("<div class='level6' title='level6'>" + article.innerHTML + "</div>"); article.remove(); } catch (e) { }
                                    ...
                                    for (articleElement of articleElements) {
                                        const article = articleElement.querySelectorAll(".article__header--law.artikel > h4")[0];
                                        try { tags.push("<div class='level5' title='level5'>" + article.innerHTML + "</div>"); article.remove(); } catch (e) { }
                                        AddTransform(articleElement);
                                    }
                                }
                            }
