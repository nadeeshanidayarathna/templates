...

        {
            ...
            var tags = await page.evaluate(function process(url) {
                ...
                try { tags.push("<div class='content'>"); } catch (e) { }
                {
                    ...
                    const mainBottomElementsTransform = mainContent.querySelectorAll(".wetsluiting");
                    const mainBottomElements = mainContent.querySelectorAll(".bijlage:not(.article__header--law)");
                    ...
                    AddTransforms(mainBottomElementsTransform);
                    for (mainBottomElement of mainBottomElements) {
                        const tableHeading = mainBottomElement.querySelectorAll(".article__header--law.bijlage > h4")[0];
                        try { tags.push("<div class='level2' title='level2'>" + tableHeading.outerHTML + "</div>"); tableHeading.remove(); } catch (e) { }
                        AddTransform(mainBottomElement.querySelectorAll(".list--law__unordered.expliciet.whitespace-small")[0]);
                        const divisies = mainBottomElement.querySelectorAll(".divisie");
                        for (divisie of divisies) {
                            const divisieHeading = mainBottomElement.querySelectorAll("h4")[0];
                            try { tags.push("<div class='level3' title='level3'>" + divisieHeading.outerHTML + "</div>"); divisieHeading.remove(); } catch (e) { }
                            AddTransform(divisie);
                        }
                    }
                }
                try { tags.push("</div>"); } catch (e) { }
