...

        {
            ...
            var tags = await page.evaluate(function process(url) {
                ...
                try { tags.push("<span class='content'>"); } catch (e) { }
                {
                    ...
                    const mainFooterElements = [];
                    Array.prototype.forEach.call(mainContent.querySelectorAll(".wetsluiting"), function (node) { mainFooterElements.push(node); });
                    Array.prototype.forEach.call(mainContent.querySelectorAll(".bijlage:not(.article__header--law)"), function (node) { mainFooterElements.push(node); });
                            ...
                            for (afdeling of afdelings) {
                                ...
                                if (paragraafs.length > 0) {
                                    for (paragraaf of paragraafs) {
                                        ...
                                        try { tags.push("<span class='level4' title='level4'>" + paraHeading.innerHTML + "</span>", "<br>"); paraHeading.remove(); } catch (e) { }
                    ...
                    for (mainFooterElement of mainFooterElements) {
                        AddTransform(mainFooterElement);
                    }
                }
                try { tags.push("</span>"); } catch (e) { }
