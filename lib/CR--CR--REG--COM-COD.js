...
const scraper = async function download(url, sp, path) {
    try {
            ...
            var tags = await page.evaluate(function process(url) {
                ...
                for(const elementContent of elementContents){
                    if(elementContent.tagName == "P"){
                        if(elementContent.tagName == "P" && elementContent.style.cssText == "text-align: center;" && elementContent.innerText.includes("PRELIMINAR")){
                            ...
                            if(child.tagName == "P" && child.style.cssText == "text-align: center;" && child.innerText.includes("LIBRO")){
                                ...
                                for(const divChild of child.childNodes){
                                    if(divChild.tagName == "P" && divChild.align == "center" && divChild.innerText.includes("LIBRO")){
                                        ...
                                        try { tags.push("<div class='level5' title='level5'>" + divChild.outerHTML + "</div>"); } catch (e) { }
                                    } else {
        ...
        await test.runTest(browser, originalHtmlPath, originalTextPath, buildHtmlPath, buildTextPath, "#divTextoPrincipal", ["#divTextoPrincipal>title", "tr>td>a"], "body", []);
        await browser.close();
    } catch (e) {
        console.log(e);
