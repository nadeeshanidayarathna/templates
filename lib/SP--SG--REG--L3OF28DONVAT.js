    ...
    try {
...

        {
            ...
            var tags = await page.evaluate(function process(url) {
                ...
                const elements = document.querySelectorAll("#barraSep > h3.documento-tit");
                ...
                const issueDate = issueDateMonth[0].slice(1, issueDateMonth[0].length - 1) + "-" + issueDateDay[0].slice(1, issueDateDay[0].length - 1) + "-" + issueDateYear;
                ...
                const rootTransformHeadings = document.querySelectorAll(".metadatos,div.redaccion > label > span , .dropdown > label > span,.enlaces-conso, #textoxslt > h3");
                AddTransforms(rootTransformHeadings);
                {
                    const elements = document.querySelectorAll("#textoxslt > div.bloque");
                    for (const element of elements) {
                        let i = 0;
                        const childs = element.childNodes;
                        for (const child of childs) {
                            if (child.className == "centro_redonda") {
                                if (i == 0) {
                                ...
                                AddTransform(child);
                            } else if (child.className == "bloque" || child.tagName == "FORM") {
                                child.remove();
                            } else if (child.className == "titulo_num") {
                                try { tags.push("<div class='level2' title='level2'>" + element.textContent + "</div>"); element.remove(); } catch (e) { }
                            } else if (child.className == "capitulo_num") {
                                try { tags.push("<div class='level3' title='level3'>" + child.textContent + " " + child.nextElementSibling.textContent + "</div>"); } catch (e) { }
                            } else if (child.className == "seccion") {
                                try { tags.push("<div class='level4' title='level4'>" + child.textContent + "</div>"); } catch (e) { }
                            } else if (child.tagName == "H5" || child.className == "anexo") {
                                try { tags.push("<div class='level5' title='level5'>" + child.textContent + "</div>"); } catch (e) { }
                            } else if (child.className != "titulo_tit" && child.className != "capitulo_tit" && child.nodeType != 3) {
                                AddTransform(child);
                            }
                        }
                    }
        ...
        await test.runTest(browser, originalHtmlPath, originalTextPath, buildHtmlPath, buildTextPath, "#textoxslt > div.bloque , .metadatos,div.redaccion > label > span ,.dropdown > label > span,.enlaces-conso, h3", ["script", ".subtitMostrado", "FORM", "p.bloque"], "body", []);
        await browser.close();
    } catch (e) {
        console.log(e);
