...
const scraper = async function download(url, sp, path) {
    try {
                    ...
                    for (const element of elements) {
                        ...
                        for (const child of childs) {
                            if (child.className == "centro_redonda") {
                                ...
                                AddTransform(child);
                            } else if (child.className == "bloque") {
                                child.remove();
        ...
        await test.runTest(browser, originalHtmlPath, originalTextPath, buildHtmlPath, buildTextPath, "#textoxslt > div.bloque , .metadatos,div.redaccion > label > span ,.dropdown > label > span,.enlaces-conso, h3", [".subtitMostrado", "p.bloque" , ".fuera", "INPUT"], "body", []);
        await browser.close();
    } catch (e) {
        console.log(e);
