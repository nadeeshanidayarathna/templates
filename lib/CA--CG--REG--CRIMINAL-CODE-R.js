            ...
            var tags = await page.evaluate(function process() {
                ...
                for (const sectionChild of sectionChilds) {
                    ...
                    if (sectionChild.tagName == "H2") {
                        ...
                        for (const scheduleChild of scheduleChilds) {
                            if (scheduleChild.tagName == "HEADER") {
                                ...
                                if (footNoteElements.length == 1) {
                                    Array.prototype.forEach.call(footNoteElements[0].querySelectorAll(".wb-invisible"), function (node) { node.remove(); }); // remove invisible tags
                                    try { tags.push("<span class='footnote' title='footnote' pos='9'>" + footNoteElements[0].innerHTML + "</span>", "<br>"); } catch (e) { }
                                    try { tags.push("<span>TODO</span>", "<br>"); } catch (e) { }
                                }
                            }
                        }
                        ...
                        Array.prototype.forEach.call(sectionChild.querySelectorAll(".wb-invisible"), function (node) { node.remove(); }); // remove invisible tags
                        try { tags.push("<span class='level3' title='level3' pos='7'>" + sectionChild.innerHTML + "</span>", "<br>"); } catch (e) { }
                        try { tags.push("<span>TODO</span>", "<br>"); } catch (e) { }
                    } else if (sectionChild.tagName == "H4") {
                        try { tags.push("<span class='level4' title='level4' pos='10'>" + sectionChild.innerHTML + "</span>", "<br>"); } catch (e) { }
                        try { tags.push("<span>TODO</span>", "<br>"); } catch (e) { }
                    }
