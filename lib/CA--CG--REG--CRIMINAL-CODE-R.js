...
const scraper = async function download(url, sp, path) {
    try {
        ...
        const { originalHtmlPath, buildHtmlPath } = await base.createFolder(url, sp, path);
...

        {
            ...
            var tags = await page.evaluate(function process() {
                ...
                Array.prototype.forEach.call(document.querySelectorAll("dt span.DefinedTerm"), function (node) { node.remove(); });
                ...
                for (const sectionChild of sectionChilds) {
                    ...
                    if (sectionChild.tagName == "H2") {
                        ...
                        if (h2MultiTags == false && nextTagName == "H3") {
                            ...
                            try { tags.push("<span class='level2' title='level2'>" + sectionChild.innerHTML); } catch (e) { }
                        } else {
                            if (sectionChild.childNodes.length == 2) {
                                try { tags.push("<span class='level2' title='level2'>" + sectionChild.childNodes[0].innerHTML + " " + sectionChild.childNodes[1].innerHTML + "</span>", "<br>"); } catch (e) { }
                            } else {
                                try { tags.push("<span class='level2' title='level2'>" + sectionChild.innerHTML + "</span>", "<br>"); } catch (e) { }
                            }
                        ...
                        for (const scheduleChild of scheduleChilds) {
                            if (scheduleChild.tagName == "HEADER") {
                                    ...
                                    if (headerChild.tagName == "H2") {
                                        ...
                                        for (const h2Child of h2Childs) {
                                            if (Array.from(h2Child.classList).includes("scheduleLabel")) {
                                                ...
                                                try { tags.push("<span class='level2' title='level2'>" + scheduleLabel + "</span>", "<br>"); } catch (e) { }
                                            } else if (!scheduleLabel.startsWith("FORM") && Array.from(h2Child.classList).includes("scheduleTitleText")) {
                                                try { tags.push("<span class='level3' title='level3'>" + h2Child.innerHTML + "</span>", "<br>"); } catch (e) { }
                                            } else {
                                ...
                                try { tags.push("<span class='level2' title='level2'>" + scheduleChild.innerHTML + "</span>", "<br>"); } catch (e) { }
                            } else if (Array.from(scheduleChild.classList).includes("ProvisionList")) {
                                const footNoteElements = scheduleChild.querySelectorAll(".Footnote > .Footnote");
                                if (footNoteElements.length == 1) {
                                    try { tags.push("<span class='level3' title='footnote'>" + footNoteElements[0].innerHTML + "</span>", "<br>"); } catch (e) { }
                                }
                            } else {
                        ...
                        try { tags.push("<span class='level3' title='level3'>" + sectionChild.innerHTML + "</span>", "<br>"); } catch (e) { }
                    } else if (sectionChild.tagName == "H4") {
                        try { tags.push("<span class='level4' title='level4'>" + sectionChild.innerHTML + "</span>", "<br>"); } catch (e) { }
                    } else {
                ...
                for (const contentSection2 of contentSection2s) {
                    ...
                    for (const contentSection2Child of contentSection2Childs) {
                        if (contentSection2Child.tagName == "H2") {
                            try { tags.push("<span class='level2' title='level2'>" + contentSection2Child.innerHTML + "</span>", "<br>"); } catch (e) { }
                        } else {
